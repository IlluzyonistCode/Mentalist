...
window.addEventListener('beforeunload', () => {
    ...
    try {
        ...
        console.log('Cleanup on unload failed:', e);
    }
});
...
async function startTracker() {
    try {
        ...
        if (result.success || (result.error && result.error.includes('запущен'))) {
            trackerStarted = true;
...
function createPlayerCard(player) {
    ...
    card.className = `player-card ${player.dead ? 'dead' : ''}`;
...
function generateCommandSuggestions(input) {
    ...
    } else if (lower.startsWith('name of')) playerNumbers.forEach(num => suggestions.push(`name of ${num} is`));
    ...
    } else if (lower.startsWith('clear')) playerNumbers.forEach(num => suggestions.push(`clear ${num}`));
